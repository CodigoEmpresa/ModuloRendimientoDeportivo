$(function(){
    ...
    $("#Certamen_Id").on('change', function(){    
        ...
        if($(this).val != ''){  
            $.get("getDivisionCertamenMetod/"+$(this).val()+"/"+$("#Metodologo_Id").val(), function (DivisionCertamen){                
                ...
                if(DivisionCertamen.length != 0){                                    
                    $.each(DivisionCertamen, function(i, e){
                        t.row.add( [
                            ...
                            '<button type="button" class="btn btn-success" onclick="AgregarMetodologo('+e['Id']+', '+$("#Metodologo_Id").val()+')">Agregar</button>',
                        ] ).draw( false );
                    });
                }
            });  
...
function AgregarMetodologo(id_certamen_division, id_persona){
    var token = $("#token").val();
    $.ajax({
      url: 'addCertamenDivisionMetodologo/'+id_certamen_division+'/'+id_persona,  
      type: 'POST',
      contentType: false,
      processData: false,
      dataType: "json",
      success: function (xhr) {
        if(xhr.Bandera == 1){                
            var t = $('#pruebasTabla').DataTable();
            t.row.add(['1','1','1'] ).clear().draw( false );
            $.get("getDivisionCertamenMetod/"+$("#Certamen_Id").val()+"/"+$("#Metodologo_Id").val(), function (DivisionCertamen){                
                if(DivisionCertamen.length != 0){                                    
                    $.each(DivisionCertamen, function(i, e){
                        t.row.add( [
                            e.division.deporte['Nombre_Deporte'],
